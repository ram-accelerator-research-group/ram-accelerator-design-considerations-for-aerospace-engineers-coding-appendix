...
T_intermediate_vec, ...
condensation_ratio_vec] = aces_cr_calc_5(alt_vec,intake_area,air_mass_flux)
...
% a series of altitudes and speeds. Results can be viewed in Chapter 1 of
...
% aces_cr_calc_5(alt_vec,intake_area,air_mass_flux) ;
...
% For 1 kg of air at each combination of stagnation pressure and
% temperature, calculate what its temperature (T_intermediate) will be when
% it transfers enthalpy to 0.7557 kg of N2 in a counterflow heat exchange
% precooler.
...
disp( ' ')
disp( 'Precooler calculations done' )

% For 1 kg of air at stagnation pressure and T_intermediate, calculate how
% much enthalpy change will be required to further cool that air to its
% saturated vapor temperature, then liquefy it.

[delta_H_to_liquefy_precooled_air_vec] = liquefy_air(T_intermediate_vec,P_air_init_vec) ;
...
% temperature to the temperature of the incoming (precooled) air.

T_offset = 1 ;

[delta_H_paraH2_Pcrit_vec] = integrate_paraH2_Pcrit_Cp_2(T_intermediate_vec,T_offset) ;

disp( ' ')
disp( 'Integration of para-H2 Cp over air liquefaction T range done' )
...
mass_paraH2_vec = abs(delta_H_to_liquefy_precooled_air_vec) ./ abs(delta_H_paraH2_Pcrit_vec) ;
